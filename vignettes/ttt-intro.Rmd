...
[mmtable1](https://github.com/ianmoran11/mmtable2),
...
content, and leaving the formatting duties to CSS, a dedicated language that
...
philosophy of not trying to all all problems, but solving some problems well.
...
general, as this vignette will demonstrate).  That it is not possible using
this package to produce all conceivable tables is a given; that was never the
intention.

## Basic Examples

Before we start, we load some packages that we will be using:
...
a good idea to include `table1` along with `ttt`.  The `magrittr` package
...
characteristic (in a regression model, we would call these dependent and
independent variables, respectively). We will assume that the data are in a
"tidy" format, by which we mean that all the *values* have been placed in a
single column (if this is not the case, there are many functions that will
allow you to "reshape" the data accordingly).
...
`<values> ~ <row facet> | <column facet>`. The first part, left of `~`, is the
column that contains the values (recall that in the "tidy" format there is only
one such column). The second part, between `~` and `|` contains the *row
facet*, one or more variables that define the row groupings of the table. The
third part, right of `|` is the *column facet*, one or more variables that
define the column groupings. (The term *facet* is borrowed from the `ggplot2`
package, where it is used in the `facet_grid()` function to describe how the
data should be split into a grid-like pattern.)

Following the `formula` comes the `data` argument, which is the
`data.frame` that contains the data that the `formula` refers to.  The
next argument `lab` is a label placed over all the columns. The last
argument, `render`, is a `function`. This function is called for each
grouping of data defined by unique combinations of the row and column
specifiers, and produces the value that appears in the corresponding
cell of the table. Hopefully this is fairly intuitive.
...
the issues that need to be addressed:
  ...
  2. One table cell contains the cryptic value "NaN" because there are
     not cars with 8 cylinders and 4 gears in our dataset; we
     would like this cell to remain empty instead.

  3. The number of decimal digits is different in each cell; we would
     like it to be the same throughout the table (say 1 decimal digit).
...
The way we addressed the first issue was to add a label to the variable
`gear` using the `label()` function. The other 2 issues were fixed by
defining a function `rndr()` to do the rendering.
...
The row facet is required by the formula syntax, but the value `1` is used
to indicate no splitting by rows:

```{r}
ttt(mpg ~ 1 | cyl, data=mtcars, lab="Number of Cylinders", render=rndr)
```

Both row and column facets can consist of more than one variable.  Here is an example with 2 row facets:
...
The order of the variables is obviously important. If we think of the `|` that
separates the row and column facets as the "values" (i.e. the central part of
the table), then the order makes sense: variables closer to the center (`|`)
are grouped within variables that are farther away.

Just to demonstrate, here is a synthetic example of a huge table where both row
...
dat$x <- 1:nrow(dat)
ttt(x ~ R3 + R2 + R1 | C1 + C2 + C3, data=dat)
```
...
Note that additional arguments can be  passed to the `render` function in `...`
...
variable (i.e., `decrease`) in the `OrchardSprays` dataset according to treatment:
...
is labelled "Statistic", but to change the label to "Blah" we can specify a
...
A caption and footnotes can be added to the table by specifying string values
to the `caption` and `footnote` arguments, respectively. There can be more than
one footnote.
...
The first this is that the whole table is enclosed in a `<div>` element with
class `Rttt`. The allows specific formatting to be applied to tables output by
`ttt` without interfering with other tables in the same document.

### Example: conditional formatting

A `render` function can actually add a `html.class` attribute to its return
value. This value of this attribute will be assigned to the resulting HTML
...
The `render` function sets the desired class on the element. Now, we can add
